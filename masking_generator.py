...
class TimeMaskingGenerator2D:
    ...
    def __call__(self):
        ...
        idx = np.random.choice(self.height, int(self.height*self.mask_ratio), replace=False)
        mask[idx, :] = 0
        mask = mask.flatten()
        return mask
...
if __name__ == "__main__":
    ...
    masked_position_generator_au = TimeMaskingGenerator2D(
                (16, 8), 0.8
            )
    masked_position_generator_au()
    print()